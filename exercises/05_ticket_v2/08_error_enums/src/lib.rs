...
#[derive(Debug)]
enum TicketNewError {
    TitleError {cause: String},
    DescriptionError {cause: String}
}
    ...
    match Ticket::new(title.clone(), description, status.clone()) {
        ...
        Err(e) => {
            match e {
                ...
                TicketNewError::TitleError { cause } => panic!("{}", cause),
            }
        }
    }
}
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::TitleError { cause: "Title cannot be empty".into() });
        }
        if title.len() > 50 {
            return Err(TicketNewError::TitleError { cause: "Title cannot be longer than 50 bytes".into()});
        }
        if description.is_empty() {
            return Err(TicketNewError::DescriptionError { cause: "Description cannot be empty".into() });
        }
        if description.len() > 500 {
            return Err(TicketNewError::DescriptionError { cause: "Description cannot be longer than 500 bytes".into() });
        }
